...
const Header = () => {
 ...
 const [userData,setUserData] = useState(null)
  ...
  useEffect(() => {
    // fetchCart();
    // if (typeof context?.fetchCart === "function") {
    //   console.log("Calling context.fetchCart");
    //   context.fetchCart();
    // }
    // fetchCart1();
    // getCate();
  ...
  // fetch cart after userData is set
  if (userData) {
    fetchCart1(userData._id);
  }
  getCate();
//     const userString = localStorage.getItem('user');
// const userData = userString ? JSON.parse(userString) : null;
// setUserData(userData)
  ...
  //   if (!userData) return;
  //   const res = await getData(`/cart/user/${userData._id}`); // <-- your backend route
  //   setCartItem(res);
  // };

  const fetchCart1 = async (userId) => {
  if (!userId) return;
  const res = await getData(`/cart/user/${userId}`);
  setCartItem(res);
};
  ...
  return (
    <div className="headerwrapp">
      ...
      <div
        className={`mobileSideBar ${isMobileSideBar !== false ? "open" : ""}`}
      >
      ...
      <div
        className={`overlay ${overlay !== false ? "open" : ""}`}
        onClick={isCloseSidebar}
      ></div>
      {/* nav bar */}
      <Navigation />
